...
SPIDER_MODULES = ['tnc.spiders']  # A list of modules where Scrapy will look for spiders.
NEWSPIDER_MODULE = 'tnc.spiders'  # Module where to create new spiders using the genspider command.

# JOBDIR = 'job_info/{}-info'.format(BOT_NAME)

# SCHEDULER = "scrapy_redis.scheduler.Scheduler"    # The scheduler to use for crawling.
# DUPEFILTER_CLASS = "scrapy_redis.dupefilter.RFPDupeFilter"    #The class used to detect and filter duplicate requests.
...
# BOT_NAME: project name to construct the User-Agent by default and logging.
# CONCURRENT_ITEMS: the maximum number of concurrent items.
# CONCURRENT_REQUESTS: the maximum number of concurrent requests.
# CONCURRENT_REQUESTS_PER_DOMAIN: the maximum number of concurrent requests of any single domain.
...
MONGODB_COLLECTION = f"china-tnc-life-190726"
